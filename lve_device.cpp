...
namespace lve {
    ...
    void LveDevice::pickPhysicalDevice() {
        ...
        for (size_t i = 0; i < devices.size(); i++) {
            ...
            std::cout << " (Type: ";
            if (props.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) std::cout << "Discrete GPU";
            else if (props.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU) std::cout << "Integrated GPU";
            else std::cout << "Other";
            std::cout << ")" << std::endl;
        }
        ...
        for (const auto& device : devices) {
            ...
            if (props.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU && isDeviceSuitable(device)) {
                ...
                std::cout << "\n✓ Selected: " << props.deviceName << " (Discrete GPU)" << std::endl;
                break;
            }
        }
        ...
        if (physicalDevice == VK_NULL_HANDLE) {
            for (const auto& device : devices) {
                if (isDeviceSuitable(device)) {
                    ...
                    VkPhysicalDeviceProperties props;
                    vkGetPhysicalDeviceProperties(device, &props);
                    std::cout << "\n✓ Selected: " << props.deviceName << std::endl;
                    break;
                }
            }
        }
